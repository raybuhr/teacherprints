...
from tp_app.main import bp
from tp_app.main.forms import EditProfileForm, EmptyForm, PostForm, SearchForm
from tp_app.models import Post, User
from tp_app.translate import translate
...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            flash(_l('User [%(username)s] is not found', username=username))
            return redirect(url_for('main.index'))
...
def search():
    if not g.search_form.validate():
        return redirect(url_from('main.explore'))
    page = request.args.get('page', 1, type=int)
